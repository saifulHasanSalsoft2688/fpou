...
const Promotor = (props) => {
    const navigate = useNavigate()

    const [formData, setFormData] = useState({})

    useEffect(() => {
        document.title = 'FPUO | Promotor Registration';
    }, [])

    const handleClick = (e) => {
        e.preventDefault()
        navigate('/complete-your-agency-profile')
    }
        ...
        <p className='text-center'>{props?.description}</p>
            <form>
                ...
                <div className="mt-4 text-center">
                    <CustomButton variant='primaryButton' text='Sign Up' onClick={handleClick} className="gradientBtn"/>
                </div>
            </form>
        </>
    )
